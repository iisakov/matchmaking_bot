...
func HandleMessageCommands(um tgbotapi.Message, b model.TgBot) model.TgBot {
	if config.MODERATORS.IsExistUserById(um.From.ID) {
		...
		case "help":
			fallthrough
		case "h":
			b.SendMsgWithKeyboardByIdAndDeleteOtherMsg(um.From.ID, model.C[b.Stage.StageType].Keyboard, um.MessageID, "Здеь будет подсказка для модераторов")
			return b
